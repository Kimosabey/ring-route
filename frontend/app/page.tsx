...
export default function Home() {
  ...
  const fetchTopology = async () => {
    try {
      const data = await fetch(getApiUrl("/api/topology")).then(r => r.json());
      setTopology(data);
    } catch (e) {
      console.error(e);
      addLog("ERROR: Failed to fetch topology. Check if port 3002 is blocked or if backend is down.");
    }
  };
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const res = await fetch(getApiUrl("/api/topology"));
  ...
  const handleAddNode = async (id: string) => {
    ...
    await fetch(getApiUrl(`/api/nodes/${id}`), { method: "POST" });
  ...
  const handleRemoveNode = async (id: string) => {
    ...
    await fetch(getApiUrl(`/api/nodes/${id}`), { method: "DELETE" });
  ...
  const handleRoute = async (key: string) => {
    ...
    const res = await fetch(getApiUrl(`/api/route?key=${key}`)).then(r => r.json());
